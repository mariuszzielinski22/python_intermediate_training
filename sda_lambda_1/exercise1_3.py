...
def exercise_2_3():
    ...
    filtered_accounts = list(filter(lambda acc: acc if acc.balance > Decimal(4500) else None, account_list))
    print(filtered_accounts)

    print("Exercise 3")

    max_accounts = max(account_list, key=lambda  acc: acc.balance)
    print(max_accounts)